...
//interrupt的用法是int when; 然后(PendingInterrupt *)pending->SortedRemove(&when);
//----------------------------------------------------------------------
...
//可以说void*功能是相当强大的。
//例如，在你设计模块间接口时，暂时无法判定传参类型。就可以通过void*传入，在函数中将指针转化你需要的类型就可以了。
